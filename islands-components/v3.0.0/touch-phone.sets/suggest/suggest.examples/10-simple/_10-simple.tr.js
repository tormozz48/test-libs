...
;
/* ../../../../touch-phone.blocks/suggest/suggest.js begin */
BEM.decl('suggest', {
    ...
    onSetMod: {
        js: function() {
            ...
            var page = this._owner.findBlockOutside('page-layout');
            ...
            this.on('update-items', function() {
                ...
                // задаем высоту паранже равной высоте всей страницы, потому что position: fixed не везде работает
                page && this._popup._under.css({height: page.domElem.height() - this._popup.domElem.offset().top});
            });

            this.channel('suggest').on('hide', this.changeThis(this._hide));
        }
    },
    ...
    _onFocus: function() {
        var _this = this,
            delta = 4; /* магическое число из спецификации
                        * http://wiki.yandex-team.ru/serp/thenewserp/dizajjn/touch.suggest#doskroll */

        // подскралливаем страницу вверх, чтобы был виден саджест
        !BEM.blocks['i-ua'].wp && $('html, body').animate({
            scrollTop: _this._owner.domElem.offset().top - delta
        });
    },
    ...
    _buildPopup: function(suggestMods) {
        return {
            ...
            underMods: {type: 'paranja', suggest: 'yes'},
            js: {
                position: {left: 0} /* Какой-то костыль… */
            },
            mix: [{block: 'suggest', mods: suggestMods}],
...
/* ../../../../touch-phone.blocks/suggest/suggest.js end */
;
...
;
/* ../../../../touch-phone.blocks/suggest-item/suggest-item.js begin */
BEM.DOM.decl('suggest-item', {
    ...
    val: function() {
        ...
        var text = this.elem('text'),
            ellipsisText = this.elem('ellipsis-text');
...
/* ../../../../touch-phone.blocks/suggest-item/suggest-item.js end */
;
...
BEM.DOM.decl('input', {
     ...
     * Передает фокус нативному контролу.
     *
     * Метод переопределен на тачах из-за ошибки с фокусом на WP8
    ...
    _focus: function() {
        ...
        // setImmediate - есть только на IE
        window.setImmediate && setImmediate(function() {
            // переприсваиваем значение нативному инпуту чтобы появилась каретка
