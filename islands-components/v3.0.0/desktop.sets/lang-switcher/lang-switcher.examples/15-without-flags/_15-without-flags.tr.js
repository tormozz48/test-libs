...
;
/* ../../../../libs/bem-bl/blocks-desktop/i-menu/i-menu.js begin */
/** @requires BEM */
/** @requires BEM */
...
/* ../../../../libs/bem-bl/blocks-desktop/i-menu/i-menu.js end */
;
...
;
/* ../../../../libs/bem-bl/blocks-desktop/b-menu-vert/b-menu-vert.js begin */
/**
 * Vertical menu
 *
 * @block b-menu-vert
...
/* ../../../../libs/bem-bl/blocks-desktop/b-menu-vert/b-menu-vert.js end */
;
...
/**
 * Управляемое выпадающее окно.
 *
 * Является составным блоком, объединяющим выпадающее окно (popup) и вызывающий его элемент,
 * относительно которого оно открывается.
 *
 * Служит для создания элементов веб-интерфейса,
 * используемых для скрытия дополнительной информации, ненужной в данный момент.
 * Например, разного рода выпадающие меню, подсказки и т.п.
 *
 * Элементом управления является элемент `switcher`.
 *
 * `switcherBlock` – имя блока, который нужно найти на элементе `switcher` (на одном узле может
 * быть несколько блоков). Если `switcherBlock` не задан, то popup показывается/скрывается при клике
 * левой кнопкой мыши в любом месте элемента `switcher`.
 *
 * Если же он задан, то открываться/закрываться popup будет при клике на блок `switcherBlock`
 * внутри элемента `switcher`.
 */
BEM.DOM.decl('dropdown', {
    /**
     * Генерируется, когда блок `popup` отображается.
     *
     * @event dropdown#show
     */

    /**
     * Генерируется, когда блок `popup` скрывается.
     *
     * @event dropdown#hide
     */
    ...
    onSetMod: {
        ...
        /**
         * При генерации события `show/hide` блоком popup, в блоке `dropdown` генерируются
         * соответствующие события и дописывается модификатор `action` со значеним `open/closed` соответственно.
         * Так же устанавливаются доступные методы блока `popup` с привязкой контекста.
         */
        'js': function() {

            var _this = this,
                params = _this.params,
                popup = _this._popup = _this
                    .findBlockOn(_this.elem(params.popupElem), 'popup')
                    .on('show', function() {
                        _this
                            .setMod('action', 'open')
                            .trigger('show');
                    ...
                    .on('hide', function() {
                        _this
                            .setMod('action', 'closed')
                            .trigger('hide');
                    });

            params.popupMethods.map(function(method) {
                _this[method] = popup[method].bind(popup);
            });

            var switcherBlock = _this.params.switcherBlock,
                switcher = _this.elem('switcher');

            if(switcherBlock) {
                _this._switcher = _this
                    .findBlockOn(switcher, switcherBlock)
                    .domElem;
            }

            /* switcher примешивается к произвольному блоку и
             * поэтому мы не можем проставить атрибуты из шаблона. */
            switcher.attr({
                'aria-haspopup': true,
                'aria-expanded': false
            });
        ...
        action: function(modName, modVal) {
            this.elem('switcher').attr('aria-expanded', modVal === 'open');
        }

    },

    /**
     * Возвращает инстанс `popup`-а.
     *
     * @public
     * @returns {BEM.DOM}
     */
    ...
    /**
     * Декларация методов блока `popup`, доступных для выполнения в блоке `dropdown`.
     *
     * @protected
     * @returns {Object} Возвращает объект, содержащий имя элемента и методы `popup`.
     */
    getDefaultParams: function() {
        return {
            popupElem: 'popup',
    ...
    /**
     * Удаляет блок, предварительно удалив слушатели событий блока `popup`.
     *
     * @protected
     */
    destruct: function() {
        this._popup && this._popup.domElem && this._popup.destruct.apply(this._popup, arguments);
        this.__base.apply(this, arguments);
    },
    ...
    _onSwitcherClick: function() {
        this._switcher || (this._switcher = this.elem('switcher'));
        this._toggle();
    },
    ...
    _onSwitcherKeyUp: function(e) {
        ...
        /* Не открываем popup по Enter для ссылок и кнопок.
         * Иначе происходит двойное срабатываение. */
        var tagName = this.elem('switcher')[0].tagName.toLowerCase();

        if(tagName === 'button') {
            return;
        }

        if((tagName !== 'a' && e.which === 13) || e.which === 32) { /* ENTER || SPACE */
            e.preventDefault();
            this.params.switcherBlock || this._toggle();
        }
    }
    ...
    live: function() {
        ...
        this
            .liveBindTo('switcher', 'leftclick tap', function(e) {
...
;
/* ../../../../common.blocks/dropdown-menu/dropdown-menu.js begin */
/**
 * Блок `dropdown-menu` – выпадающее меню.
 */
BEM.DOM.decl({
    block: 'dropdown-menu',
    baseBlock: 'dropdown'
...
/* ../../../../common.blocks/dropdown-menu/dropdown-menu.js end */
;
/* ../../../../libs/bem-bl/blocks-desktop/b-menu-horiz/b-menu-horiz.js begin */
(function() {
...
/* ../../../../libs/bem-bl/blocks-desktop/b-menu-horiz/b-menu-horiz.js end */
;
/* ../../../../common.blocks/lang-switcher/lang-switcher.js begin */
/**
 * Переключатель языка.
 *
 * На этапе инициализации выполняется предварительная загрузка
 * изображения для каждого дочернего элемента `flag`.
 * Пути к изображениям берутся из CSS-свойства `background-image`.
 */
BEM.DOM.decl('lang-switcher', {
    _onLangKeyUp: function(e) {
        var elem = e.data.domElem;

        if(this.hasMod(elem, 'selected', 'yes') ||
           !this.findBlockOn('popup').containsDomElem(elem) ||
    ...
    live: function() {
        ...
        /* Делаем pre-fetch флагов, для более быстрого отображения. */
        $(this.buildSelector('flag')).each(function() {
            new Image().src = $(this).css('backgroundImage').match(/url\(["']?([^'")]+)["']?\)/)[1];
        });
...
/* ../../../../common.blocks/lang-switcher/lang-switcher.js end */
;
/* ../../../../common.blocks/lang-switcher/lang-switcher.examples/15-without-flags.blocks/lang-switcher/lang-switcher.js begin */
BEM.DOM.decl('lang-switcher', {
    ...
    live: function() {
        this.liveInitOnBlockEvent('init', 'dropdown');
    }

});

/* ../../../../common.blocks/lang-switcher/lang-switcher.examples/15-without-flags.blocks/lang-switcher/lang-switcher.js end */
;
...
BEM.I18N.decl('i-services', {"404": '404',"adresa": 'Adresler',"advertising": 'Reklamlar hakkında',"afisha": 'Afiş',"all": 'Tüm servisler',"api": 'API',"appsearch": 'Uygulamalar',"auto": 'Oto',"avia": 'Uçak biletleri',"aziada": 'Азиада',"ba": 'Bayan',"backapv": 'Партнер Я.Карт',"balance": 'Bakiye',"bar": 'Bar',"bar-ie": 'Бар для ИЕ',"bar-ie9": 'Бар для ИЕ9',"bayan": 'Баннеры Яндекса',"blogs": 'Bloglar',"books": 'Kitaplar',"browser": 'Browser',"calendar": 'Takvim',"captcha": 'ой...',"catalogwdgt": 'Widget kataloğu',"chrome": 'Хром с поиском Яндекса',"city": 'Şehirler',"cityday": 'Şehrin kuruluş günü',"collection": 'Koleksiyon',"company": 'Şirket',"contest": 'Contest',"desktop": 'Kişisel arama',"direct": 'Direkt',"direct.market": 'Market',"disk": 'Disk',"ege": 'ЕГЭ',"expert": 'Эксперт',"feedback": 'Bize ulaşın',"feedback2": 'Bize ulaşın',"ff": 'ФФ с поиском Яндекса',"fotki": 'Fotoğraflar',"fresh": 'Güncel',"games": 'Oyunlar',"geocontext": 'Геоконтекст',"goroda": 'Şehirler',"help": 'Yardım',"i": 'Servislerim',"ie": 'ИЕ с поиском Яндекса',"images": 'Görsel',"images-com": 'Görsel',"interests": 'İlgi alanları',"internet": 'Интернет',"keyboard": 'Klavye',"kraski": 'Renkler',"kuda": 'Herkes nereye gidiyor',"large": 'Görme bozukluğu olanlar için Yandex',"legal": 'Yasal belgeler',"lenta": 'Feeds',"libra": 'Библиотека',"literacy": 'Неделя борьбы за грамотность',"local": 'Lokal ağ',"lost": 'Önceki servisler',"love": 'Karşılıklı çekim günü',"mail": 'Mail',"maps": 'Haritalar',"maps-wiki": 'Kullanıcı haritası',"market": 'Market',"market.advertising": 'Market',"metrika": 'Metrika',"metro": 'Metro',"mobile": 'Mobil',"moikrug": 'Moy Krug',"money": 'Money',"museums": 'Tarihi Kültür Mirası Günleri',"music": 'Müzik',"music-partner": 'Müzik: İstatistik',"nahodki": 'Bulduklarım',"nano": 'Nano',"newhire": 'Yeni kiralama',"news": 'Haberler',"notifications": 'Bildirim paneli',"oauth": 'Giriş',"online": 'Çevrimiçi',"openid": 'OpenID',"opera": 'Opera Software',"opinion": 'Alıntılar',"partners": 'Reklam ağı',"partnersearch": 'Partnerler için arama',"passport": 'Pasaport',"pdd": 'Alanlar için posta',"peoplesearch": 'Kişi arama',"perevod": 'Çeviri',"probki": 'Trafik',"pulse": 'блоги: пульс',"punto": 'Punto switcher',"pvo": 'Yanıtlar',"rabota": 'Kariyer',"ramazan": 'ramazan',"rasp": 'Seferler',"realty": 'Emlak',"referats": 'Tezler',"rk": 'Sorularım var',"root": 'Unix kullanıcıları için Yandex.Olimpiyat',"school": 'Школа',"search": 'Arama',"server": 'Server',"site": 'Site için arama',"slovari": 'Sözlükler',"so": 'Самооборона',"social": 'Социализм',"soft": 'Uygulamalar',"sport": 'Spor',"sprav": 'Rehber',"start": 'Başlangıç sayfası',"stat": 'İstatistikler',"subs": 'Abonelikler',"taxi": 'Такси',"terms": 'İnternet rehberi',"tests": 'Testler ve anketler',"tickets": 'Biletler',"time": 'Yandex.Saat',"toster": 'Kutlama konuşması',"translate": 'Çeviri',"tune": 'Ayarlar',"tv": 'TV Rehberi',"uslugi": 'Услуги',"video": 'Video',"video-com": 'Video',"vno": 'ZNO',"wdgt": 'Widgets',"weather": 'Hava durumu',"webmaster": 'Webmaster',"widgets": 'Widgets',"wordstat": 'İstatistikler',"wow": 'Ya.ru',"www": 'Web',"xmlsearch": 'XML',"yaca": 'Katalog',"yamb": 'Медийные баннеры',"zakladki": 'Favoriler'}, {
"lang": "tr"
});

BEM.I18N.decl('lang-switcher', {"all": 'Daha'}, {
"lang": "tr"
});
