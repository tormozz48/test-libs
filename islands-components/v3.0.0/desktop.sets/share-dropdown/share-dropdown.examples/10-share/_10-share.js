...
;
/* ../../../../common.blocks/share-dropdown/share-dropdown.js begin */
/**
 * Блок «Поделиться».
 *
 * Блок `share-dropdown`, cлужит оберткой для блока «Поделиться», основан на блоке `dropdown-menu`.
 ...
 */
BEM.DOM.decl('share-dropdown', {

    /**
     * Обновление данных для указанного сервиса.
     *
     * @public
     * @param  {String} service Имя сервиса, соответствующее выставленному модификатору (`facebook`, `moimir`, etc.).
     * @param  {Object} params Параметры, по которым строится ссылка для сервиса.
     * @param  {String} [params.service] Сервис. Если данный параметр не указан, то берется
     * значение data-атрибута `data-service` из DOM-элемента блока `link`. Иначе, переданное
     * значение параметра должно соответствовать первому параметру (`service`).
     * @param  {String} [params.url] Адрес страницы, которой необходимо поделиться. Значение по умолчанию берется из
     * window.location.href.
     * @param  {String} [params.title] Заголовок страницы. Значение по умолчанию - document.title.
     * @param  {String} [params.image] Дополнительная картинка, передаваемая в сервис. Для Pinterest этот
     * параметр является обязательным.
     * @param  {String} [params.description] Описание ссылки. Если параметр не указан,
     * описания берется из мета-тега description.
     * @returns {BEM.DOM} Экземпляр блока.
     */
    updateLink: function(service, params) {

        var blink = this.findBlockInside(this.elem('share-item', 'service', service), 'link');

        this._updateLink(blink, params);
    ...
    /**
     * Обновление данных для всех сервисов указанных в блоке.
     *
     * @public
     * @param  {Object} params Параметры, по которым строится ссылка для сервиса.
     * @param  {String} [params.service] Сервис. Если данный параметр не указан, то берется
    ...
    updateLinks: function(params) {
        ...
        this.findBlocksInside('link').forEach(

            function(blink) {

                _this._updateLink(blink, params);
     ...
     * @private
     */
    _updateLink: function(blink, params) {

        var blinkDom = blink.domElem;
        params = params || {};

        blinkDom.attr(
            'href',
            '//share.yandex.ru/go.xml?service=' +
                ( params.service ? params.service : blinkDom.attr('data-service') ) +
                '&url=' + encodeURIComponent( (params.url !== undefined) ? params.url : window.location.href ) +
                '&title=' + encodeURIComponent( params.title ? params.title : document.title ) +
...
/* ../../../../common.blocks/share-dropdown/share-dropdown.js end */
;
...
BEM.I18N.decl('share-dropdown', {"facebook": 'Facebook',"gplus": 'Google +',"moimir": 'Мой Мир',"odnoklassniki": 'Одноклассники',"pinterest": 'Pinterest',"share": 'Поделиться',"twitter": 'Twitter',"vkontakte": 'Вконтакте'}, {
"lang": "ru"
});
