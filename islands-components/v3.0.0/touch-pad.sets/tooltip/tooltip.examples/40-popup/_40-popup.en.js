...
;
/* ../../../../common.blocks/tooltip/tooltip.examples/40-popup.blocks/example/example.js begin */
        ...
        'js': {
            'inited': function() {
                var button = this.findBlockInside('button'),
                    ...
                    owner = popup.findBlockInside('tooltip-owner'),
                    tooltip = popup.findBlockInside('tooltip');
...
/* ../../../../common.blocks/tooltip/tooltip.examples/40-popup.blocks/example/example.js end */
;
...
;
/* ../../../../common.blocks/tooltip/tooltip.js begin */
(function($) {

var bro = $.browser,
    //такое же событие как в popup_autoclosable
    KEYDOWN_EVENT = (bro.opera && bro.version < 12.10) ? 'keypress' : 'keydown';

/**
 * Всплывающая подсказка-указатель.
 */
BEM.DOM.decl('tooltip', {
    ...
    /**
     * Герерируется при показе подсказки.
     *
     * @event tooltip#show
     */

    /**
     * Генерерируется при скрытии подсказки.
     *
     * @event tooltip#hide
     */
    ...
    onSetMod: {
        ...
        'shown': {
            'yes': function() {
                if(!this._owner) {
                    throw new Error('Owner is unset');
                }
                this._getPopup()
                    .show(this._owner)
                    // Попап всегда закрывается по ESC
                    // FIXME: http://st.yandex-team.ru/ISLCOMPONENTS-1034
                    .unbindFromDoc(KEYDOWN_EVENT);
            },
            '': function() {
                this._getPopup().hide();
            }
    ...
    /**
     * Возвращает объект на который указывает подсказка.
     *
     * @public
    ...
    /**
     * Определяет объект на который будет указывать подсказка.
     *
     * ```js
     * tooltip
     *  .setOwner(ownerBlock)
    ...
    _onPopupShow: function() {
        // NOTE: события в `popup` инициируются асинхронно,
        // поэтому унести события `tooltip` в обработчик модификатора мы не можем
        this.trigger('show');
    },
...
/* ../../../../common.blocks/tooltip/tooltip.js end */
;
/* ../../../../common.blocks/tooltip/_autoclosable/tooltip_autoclosable_yes.js begin */
(function() {

var KEY_ESC = 27;

/**
 * Подсказка, автоматически закрывающаяся при клике за ее пределами,
 * а также по нажатию на клавишу ESC.
 */
BEM.DOM.decl({block: 'tooltip', modName: 'autoclosable', modVal: 'yes'}, {
    onSetMod: {
        'shown': {
            'yes': function() {
                this.__base.apply(this, arguments);
...
/* ../../../../common.blocks/tooltip/_autoclosable/tooltip_autoclosable_yes.js end */
;
