...
;
/* ../../../../common.blocks/spin/spin.examples/20-progress.blocks/example/example.js begin */
BEM.DOM.decl('example', {
    onSetMod: {
        js: {
            inited: function() {
                var spin = this.findBlockInside('spin'),
                    checkButton = this.findBlockInside('check-button');

                checkButton.on('change', function() {
                    spin.toggleMod('progress', 'yes', '', checkButton.hasMod('checked', 'yes'));
                });
            }
        }
    }
...
/* ../../../../common.blocks/spin/spin.examples/20-progress.blocks/example/example.js end */
;
...
// Private-константы использующиеся внутри блока.
var CSS_BGPOS_PROPERTY = 'background-position', // Имя CSS свойства для анимации с помощью CSS-sprite.
    CSS_BGPOS_PREFIX = '0 -',                   // Префикс для значения CSS свойства 'background-position'.
    CSS_ANIMATION_SUPPORTED = false,            // Устанавливается в true если браузер поддерживает CSS3 animation.
    CSS_ANIMATION_PROPERTY = 'animation',    // Имя CSS свойства 'animation', возможно, с вендорным префиксом.
    CSS_ANIMATION_NAME = 'islands-spin',     // Имя анимации из правила @keyframes.
    PARAM_ANIMATION_TRANSFORM = 'transform', // Значение js-параметра animation: анимация с помощью CSS3 animation.
    PARAM_ANIMATION_SPRITE = 'sprite';       // Значение js-параметра animation: анимация с помощью CSS-sprite.

/**
 * Исправляет проблему с 'background-position' в IE.
 * Проверяет поддержку браузером CSS свойства animation.
 * Устанавливает значения констант CSS_ANIMATION_SUPPORTED и CSS_ANIMATION_PROPERTY.
 */
(function() {
    var elm = document.createElement('img');

    // Фикс для IE
    if(elm.style['background-position-y'] !== undefined) {
        CSS_BGPOS_PROPERTY = 'background-position-y';
        CSS_BGPOS_PREFIX = '-';
    }
    ...
    if(elm.style.animationName !== undefined) {
        ...
        for(var i = 0; i < domPrefixes.length; i++) {
            ...
            if(elm.style[pfx + 'AnimationName'] !== undefined) {
                CSS_ANIMATION_PROPERTY = '-' + pfx.toLowerCase() + '-' + 'animation';
                CSS_ANIMATION_SUPPORTED = true;
                break;
            }
        }
...
/**
 * Индикатор загрузки.
 * Вращается посредством CSS3 анимации, в старых браузерах деградирует до CSS-sprite анимации.
 *
 * @param {Object} [params]                 Параметры блока:
 * @param {String} [params.animation]       Вид анимации: 'transform' для CSS3 анимации,
 *                                                        'sprite' для анимации через CSS-sprite.
 * @param {Number} [params.startDelay=0]    Задержка (ms) перед стартом анимации.
 * @param {Number} [params.frameAngle=10]   Угол поворота (deg) за один кадр. Только для CSS3 анимации.
 * @param {Number} [params.frameTime=27]    Время отображения (ms) одного кадра анимации.
 * @param {Number} [params.frames=36]       Количество кадров анимации. Только для анимации через CSS-sprite.
 */
BEM.DOM.decl('spin', {
    /**
    ...
    getDefaultParams: function() {
        return {
            animation: PARAM_ANIMATION_TRANSFORM,
            startDelay: 0,
            frameAngle: 10,
            frameTime: 27,
            frames: 36
    ...
    onSetMod: {
        /**
         * Вычисляет размеры блока для CSS-sprite анимации.
         * Если у блока модификатор progress установлен yes, анимация запускается сразу.
         */
        'js': function() {
            // Сбрасывает способ анимации в CSS-sprite если CSS свойство animation не поддерживается
            if(!CSS_ANIMATION_SUPPORTED) {
                this.params.animation = PARAM_ANIMATION_SPRITE;
            }

            if(this.params.animation === PARAM_ANIMATION_SPRITE) {
                this._size = parseInt(this.getMod('size') || /[\d]+/.exec(this.getMod('theme'))[0], 10);
                this._height = this._size * this.params.frames;
                this._curFrame = 0;

                this.setMod('support-transforms', 'no');
            }

            if(this.hasMod('progress', 'yes')) {
                this._startSpin();
            }
        },
    ...
    _startSpin: function() {
        var commence = function() {
            if(this.hasMod('progress', 'yes') && !this._interval) {
                switch(this.params.animation) {
                    case PARAM_ANIMATION_TRANSFORM:
                        // Вычисляем сколько миллисекунд занимает полный оборот.
                        var animationStr = CSS_ANIMATION_NAME + ' ' +
                            Math.floor(this.params.frameTime * 360 / this.params.frameAngle) +
                            'ms infinite linear';
                        this.elem('icon').css(CSS_ANIMATION_PROPERTY, animationStr);
                        break;

                    case PARAM_ANIMATION_SPRITE:
                        // `this.params.frameTime` миллисекунд на кадр анимации.
                        this._interval = setInterval(this._switchFrame.bind(this), this.params.frameTime);
                        break;
                }
            }
        }.bind(this);

        this._delayTimeout = setTimeout(commence, this.params.startDelay);
    },
    ...
    /**
     * Показывает следующий фрейм для элемента 'icon' во время CSS-sprite анимации.
     *
     * @private
     */
    _switchFrame: function() {
        var curFrame = ++this._curFrame,
            y = curFrame * this._size;

        if(y >= this._height) {
            this._curFrame = y = 0;
        }
        this.elem('icon').css(CSS_BGPOS_PROPERTY, CSS_BGPOS_PREFIX + y + 'px');
    },
