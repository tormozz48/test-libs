...
;
/* ../../../../touch-pad.blocks/radiobox/radiobox.js begin */
...
/* ../../../../touch-pad.blocks/radiobox/radiobox.js end */
;
...
;
/* ../../../../common.blocks/tabs/_control/tabs_control_buttons.js begin */
BEM.DOM.decl({block: 'tabs', modName: 'control', modVal: 'buttons'}, {
    ...
    onSetMod: {
        js: function() {
            this.__base.apply(this, arguments);
            this._radioButton = this.findBlockOn('control', 'radio-button');
        }
    },

    activate: function(index) {
        if(!this.isDisabled(index)) {
            this._radioButton.setMod(this._getRadio(index), 'checked', 'yes');
        }
    ...
    disable: function(index) {
        this._radioButton.setMod(this._getRadio(index), 'disabled', 'yes');

        return this;
    },

    enable: function(index) {
        this._radioButton.delMod(this._getRadio(index), 'disabled');

        return this;
    },

    isDisabled: function(index) {
        return this._radioButton.isDisabled(this._getRadio(index));
    },

    getActive: function() {
        return this._indexOfButton(this._radioButton.getCurrent());
    },

    /**
     * Возвращает DOM-элемент радиокнопки по индексу.
     *
     * @private
     * @param {Number} index Индекс кнопки. Индексация начинается с 0.
     * @returns {jQuery}
     */
    _getRadio: function(index) {
        return this._radioButton.elem('radio').eq(index);
    },

    /**
     * Возвращает индекс кнопки или -1, если переданный элемент не найден среди кнопок.
     *
     * @private
     * @param {jQuery} elem DOM-элемент кнопки.
     * @return {Number}
     */
    _indexOfButton: function(elem) {
        return $.inArray(elem.get(0), this._radioButton.elem('radio'));
    },

    _onControlChange: function(e, data) {
        this.trigger('change', {
            currentIndex: this._indexOfButton(data.current),
            prevIndex: this._indexOfButton(data.prev)
        });
    }
});

/* ../../../../common.blocks/tabs/_control/tabs_control_buttons.js end */
;
