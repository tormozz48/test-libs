...
;
/* ../../../../common.blocks/tabs/tabs.examples/40-api.blocks/example/example.js begin */
BEM.DOM.decl('example', {
    onSetMod: {
        js: function(){
            var tabsMenu = this.findBlockInside('tabs');

            this.elem('action').each(function(index, elem) {
                elem = $(elem);

                var params = this.elemParams(elem);
                params[1] || (params[1] = []);

                this.findBlockOn(elem, 'button').on('click', function() {
                    var res = tabsMenu[params[0]].apply(tabsMenu, params[1]);
                    params[2] && this.log('<code>' + res + '</code>');
                }, this);
            }.bind(this));

            tabsMenu.on('change', function(e, data) {
                this.log('<b>tabs#change:</b> <code>{currentIndex: ' + data.currentIndex + ', prevIndex: '
                    + data.prevIndex + '}</code>');
            }, this);
        }
    },
...
/* ../../../../common.blocks/tabs/tabs.examples/40-api.blocks/example/example.js end */
;
/* ../../../../common.blocks/tabs/tabs.js begin */
(function() {
...
BEM.DOM.decl('tabs', {
    ...
    /**
     * Генерируется при смене активного таба.
     *
     * @event tabs#change
     * @param {Object} data              Данные события.
     * @param {Number} data.currentIndex Индекс активного таба.
     * @param {Number} data.prevIndex    Индекс предыдущего активного таба.
     */

    /**
     * Активирует табик по индексу.
     *
     * @public
     * @param {Number} index Индекс табика. Индексация начинается от 0.
    ...
    /**
     * Возвращает индекс активного табика или -1, если нет активного табика. Индексация начинается от 0.
     *
     * @public
     * @returns {Number}
     */
    ...
    /**
     * Проверяет, задизейблен ли табик.
     *
     * @public
     * @param {Number} index Индекс табика. Индексация начинается от 0.
     * @returns {Boolean}
    ...
    /**
     * Возвращает список блоков, которые должны вызывать инициализацю блока `tabs` своим событием `change`.
...
/* ../../../../common.blocks/tabs/tabs.js end */
;
...
;
/* ../../../../common.blocks/tabs-menu/tabs-menu.js begin */
BEM.DOM.decl('tabs-menu', {

    /**
     * Генерируется при смене активного табика.
     *
     * @event tabs-menu#change
     * @param {Object} data              Данные события.
     * @param {jQuery} data.current      Активированный табик.
     * @param {jQuery} data.prevIndex    Деактивированный табик.
     */
    ...
    onElemSetMod: {
        tab: {
            active: {
                yes: function(elem) {
                    if(this.hasMod(elem, 'disabled', 'yes')) {
                        return false;
                    }
            ...
            disabled: function(elem, modName, modVal) {
                this.findBlockOn(elem, 'link').setMod('disabled', modVal);
            }
        }
    },

    /**
     * Возвращает элемент активного табика.
     *
     * @public
     * @returns {jQuery}
     */
    ...
    /**
     * Возвращает DOM-элемент табика по его индексу.
     *
     * @public
     * @param {Number} index Индекс табика. Индексация начинается от 0.
     * @returns {jQuery}
    ...
    getTab: function(index) {
        return this.elem('tab').eq(index);
    },

    /**
     * Возвращает индекс табика или -1, если переданный элемент не найден среди табиков.
     *
     * @public
     * @param {jQuery} [elem] DOM-элемент табика.
     * @return {Number}
     */
    indexOfTab: function(elem) {
        return $.inArray(elem.get(0), this.elem('tab'));
    },

    /**
     * Обработчик клика по табику.
     *
     * @private
     * @param {$.Event} e
     */
    _onClick: function(e) {
        e.preventDefault();
        this.setMod($(e.target), 'active', 'yes');
    }
}, {
    live: function() {
...
/* ../../../../common.blocks/tabs-menu/tabs-menu.js end */
;
...
;
/* ../../../../common.blocks/tabs/_control/tabs_control_menu.js begin */
BEM.DOM.decl({block: 'tabs', modName: 'control', modVal: 'menu'}, {
    onSetMod: {
        js: function() {
            this.__base.apply(this, arguments);
    ...
    _onControlChange: function(e, data) {
        this.trigger('change', {
            currentIndex: this._tabsMenu.indexOfTab(data.current),
            prevIndex: this._tabsMenu.indexOfTab(data.prev)
        });
    }
});

/* ../../../../common.blocks/tabs/_control/tabs_control_menu.js end */
;
/* ../../../../common.blocks/tabs-panes/tabs-panes.js begin */
BEM.DOM.decl('tabs-panes', {
        ...
        pane: {
            active: {
                yes: function(elem, modName, modVal) {
                    this.delMod(this.findPane('active', 'yes'), 'active');
                }
            }
        }
    },
     ...
     *
     * Рекомендуется исаользовать именно этот метод для получения панелек, т.к. он возвращает только панельки,
...
/* ../../../../common.blocks/tabs-panes/tabs-panes.js end */
;
