...
;
/* ../../../../common.blocks/suggest/suggest.examples/10-simple.blocks/example/example.js begin */
...
/* ../../../../common.blocks/suggest/suggest.examples/10-simple.blocks/example/example.js end */
;
...
;
/* ../../../../common.blocks/suggest/suggest.js begin */
/**
 * Саджест – поисковая подсказка.
 *
 * Появляется после первых введенных букв при формулировке поискового запроса.
 * Состоит из элементов поисковой подсказки (блок suggest-item).
 * Используется для экономии времени на формулировку запроса.
 * Блок не имеет DOM-представления, создаётся и подключается с помощью BEM.create()
 *
 * После ввода хотя бы одного символа или клика в поле ввода поискового запроса
 * создаётся блок `popup` (DOM-узлы) и вся работа с DOM идёт через него.
 *
 * @param {BEM}      params.owner            Блок для которого появляются подсказки.
 * @param {BEM}      params.input            Блок ввода для запроса, например блок input.
 * @param {String}   params.url              Адрес сервера, источника данных для подсказок.
 * @param {String}  [params.name]            Название дата-провайдера, который подгототавливает данных от сервера.
 * @param {Object}  [params.suggest]         Дополнительные параметры для подсказок http://nda.ya.ru/3QTLT9.
 * @param {Object}  [params.mods]            Модификаторы саджеста, передаваемые в popup в котором строятся подсказки.
 * @param {String}  [params.label]           Указатель на блок service-label.
 * @param {Boolean} [params.cache]           Флаг, указывающий кешировать или нет результаты запроса к серверу.
 * @param {Boolean} [params.updateOnEnter]   Обновлять строку-запрос при перемещении по элементам саджеста клавишами ↑↓.
 * @param {Number}  [params.debounceDelay]   Таймаут на изменения подсказки при быстром наборе запроса, ms.
 * @param {Boolean} [params.showListOnFocus] Флаг показа списка подсказок по фокусу мыши.
 */

BEM.decl('suggest', {

   /**
    * Генерируется перед отправкой данных.
    *
    * @event suggest#data-requested
    */

    /**
     * Генерируется после создания подсказки.
     *
     * @event suggest#created
     */

    /**
     * Генерируется после получения данных.
     *
     * @event suggest#data-received
     * @param {Object} data Данные, полученные от провайдера.
     */

    /**
     * Генерируется в момент, когда был показан новый набор подсказок.
     *
     * @event suggest#update-items
     */

    /**
     * Генерируется в момент выбора элемента, строки подсказки.
     *
     * @event suggest#select
     * @param {BEM.DOM}  item            Выбранный элемент подсказки, suggest-item.
     * @param {Number}   itemIndex       Позиция подсказки в массиве подсказок.
     * @param {Number}   pos             Позиция курсора-каретки в поле ввода на последний запрос к серверу.
     * @param {Object}   meta            Мета-информация, полученная от сервера при последнем запросе.
     * @param {Boolean}  byKeyboard      Флаг, выбрана ли подсказка при помощи клавиатуры.
     */
    ...
    onSetMod: {
        js: function() {
            ...
            _this._onChange = $.debounce(_this._onChange, _this.params.debounceDelay);

            _this._owner = _this.params.owner;
            input = _this._input = _this.params.input;
            control = _this._control = input.elem('control');
            _this._label = _this.params.label;

            /* Кешируется для постоянного сравнения с activeNode. */
            _this._control0 = control[0];

            /* Данные на момент открытия страницы. */
            _this._val = input.val();

            /* Перед отключение autocomplete в браузере необходимо снять
             * фокус с поля ввода. */
            focused = input.hasMod('focused');
            focused && input.delMod('focused');
            control.attr({
                'autocomplete': 'off',
                'autocorrect': 'off',
                'autocapitalize': 'off',
            ...
            focused && input.setMod('focused', 'yes');

            input
                .on('change', _this._onChange, _this)
                .on('focus', _this._onFocus, _this)
                .on('blur', _this._onBlur, _this);

            control
                .keydown(_this._onKeyDown.bind(_this))
                .keypress(_this._onKeyPress.bind(_this))
                .keyup(_this._onKeyUp.bind(_this)); /* TODO Острова 2.0: Отказаться. */
        }
    },

    /**
     * Уничтожает сам блок.
     * Уничтожает связанный блок popup.
     *
     * @public
     */
    destruct: function() {
    ...
    /**
     * Возвращает параметры блока по умолчанию с добавлением параметров
     * cache, updateOnEnter, debounceDelay, showListOnFocus.
     *
     * @protected
     * @returns {Object}
     */
    ...
    /**
     * Возвращает блок всплывающего окошка.
     *
     * Обычно это блок popup.
     *
     * @public
     * @returns {BEM.DOM}
     */
    ...
    /**
     * Обновляет позицию окошка на странице.
     *
     * Окошко всегда позиционируется под шапкой owner.
     *
     * @returns {Block}
     */
    updateTop: function() {
        var owner = this._owner.domElem,
            _top = owner.offset().top + owner.outerHeight();

        /* На заметку: это значение не содержит высоту тени-рамки. */
        this._popup.domElem.css('top', _top);

        return this;
    },

    /**
     * Обновить окошко с подсказками после асинхронной перезагрузки страницы.
     *
     * @public
     * @returns {Block}
     */
    redraw: function() {
        ...
        if(label) {
            ...
            /* Очистка предыдущей коррекции css-стилей в `<head>` для `suggest-item` и `suggest__title` */
            _this._itemStyle && _this._itemStyle.remove();

            var body = $('body'),
                labelWidth = label.outerWidth(true),
                inputControl = _this._input.elem('control'),
                itemNewPaddingLeft = inputControl.offset().left + parseInt(inputControl.css('padding-left'), 10),
                //TODO синхронизировать версии islands-search
                offset = parseInt(_this._input.findBlockInside(label, 'link').domElem.css('padding-right'), 10)
                         - parseInt(_this._owner.findBlockInside('arrow').domElem.css('padding-left'), 10),
                titleElem;

            /* titleElem нужен для вычисления ширины заголовка подсказки  */
            titleElem = $('<div class="suggest__title" style="position:absolute;visibility:hidden"/>').appendTo(body);

            /* Коррекция css-стилей в `<head>` для `suggest-item` и `suggest__title` */
            _this._itemStyle = $(BEMHTML.apply({
                block: 'suggest',
                elem: 'padding',
                itemPaddingLeft: itemNewPaddingLeft,
                titleWidth: titleElem.width() + labelWidth - offset
            })).appendTo($('head'));
    ...
    _create: function() {
        ...
        var _self = _this.__self,
            BEMJSON = _self._buildPopup(_this.params.mods),
            $html = $(BEMHTML.apply(BEMJSON));

        BEM.DOM.append(BEM.DOM.scope, $html);

        _this._popup = $html.bem('popup');

        this.params.needUpdateTop && _this.updateTop();
    ...
    _request: function() {
        ...
        val = _this._input.val();
        pos = _this._getPos();

        _this._val = val;
        _this._pos = pos;

        /* Запрос будет отправлен даже при пустом вводе. Сервер может
         * прислать персональные подсказки. */
        _this.trigger('data-requested')
    ...
    _onData: function(data) {
        ...
        BEMJSON = this.__self._buildItems(items, meta);
        popup.setContent(BEMHTML.apply(BEMJSON));

        this._show();
        this._items = popup.findBlocksInside('suggest-item');
     ...
     *
     * Провайдер представлен блоком suggest__provider.
     *
     * @protected
     * @returns {BEM}
     */
    _getProvider: function() {
        ...
        name = params.name || _this.__self.getName() + '__provider';

        data = $.extend({}, {
            v: 4,
            yu: BEM.blocks['i-global'].param('yandexuid'),
            fact: 1,
            wiz: 'TrWhr'
        }, params.suggest);

        prov = _this._provider = BEM.create(name, {
            url: params.url,
            data: data,
            cache: params.cache,
            dataType: params.json ? 'json' : 'jsonp',
            callbackCtx: _this
        });
    ...
    _onChange: function(e, data) {
        if(data && data.noSuggest) {
            return;
        }

        if(data && data.source !== 'suggest') {
            this._hide();
            this._clear();
            return;
        }
    ...
    _onFocus: function() {
        ...
        _this.params.showListOnFocus && _this._items.length &&
            _this._input.val() && _this._show();
    },
    ...
    _onKeyDown: function(e) {
        ...
        /* Отправляем данные с новым значением pos. */
        /*LEFT/RIGHT*/
        if(which === 37 || which === 39) {
            _this.afterCurrentEvent(_this._request, _this);
        }

        if(which !== 38 && which !== 40 || e.shiftKey) {
            return;
        }

        e.preventDefault();

        len = _this._items.length;
        if(!len) {
            return;
        }
        ...
        if((idx === 0 && dir === -1) || (idx + dir >= len)) {
            _this._onLeaveItem(_this._items[idx], true);
            return;
        }

        idx += dir;
        idx = (idx < 0) ? (len - 1) : (idx >= len) ? 0 : idx;

        _this._onEnterItem(_this._items[idx], true);
    },
    ...
    _onKeyPress: function(e) {
        ...
        /* Далее идут действия на нажатие Enter. */

        var _this = this,
            idx = _this._idx;

        if(idx > -1 && _this._byKeyboard) {
            e.preventDefault();
            _this._onSelectItem(_this._items[idx], true);
        }

        _this._hide();
    },

    /* TODO Острова 2.0: Отказаться. */
    _onKeyUp: function(e) {
    },
    ...
    _onEnterItem: function(item, byKeyboard) {
        ...
        if(idx > -1) {
            items[idx].delMod('selected');
        }
        idx = _this._getIdx(item);
        if(idx > -1) {
            items[idx].setMod('selected', 'yes');
        }

        _this._idx = idx;
        _this._byKeyboard = byKeyboard;

        _this._control.attr('aria-activedescendant', item.domElem.attr('id'));

        if(!byKeyboard || !_this.params.updateOnEnter) {
            return;
        }

        if(item.enter() !== false) {
            _this._input.val(item.val(), {
                source: 'suggest',
                noSuggest: true,
                item: item
            });
    ...
    _onLeaveItem: function(item, byKeyboard) {
        ...
        if((idx > -1) && (idx === _this._getIdx(item))) {
            _this._items[idx].delMod('selected');
            _this._idx = -1;

            _this._control.removeAttr('aria-activedescendant');
        }

        /* Если при помощи клавиатуры мы увели курсор из списка, то
         * заполняем поле ранее введённым значением. */
        if(byKeyboard) {
            _this._input.val(_this._val, {
                source: 'suggest'
            });
        }
    },
    ...
    _onSelectItem: function(item, byKeyboard) {
        var _this = this,
            needUpdate = item.select(byKeyboard);

        /* Сначала обновляем поле ввода, потом отправляем уведомление и
         * только потом занимаемся очисткой. Порядок важен! */
        _this._val = item.val();
        ...
        _this.trigger('select', {
            item: item,
            itemIndex: _this._idx,
            pos: _this._pos,
            meta: _this._meta,
            byKeyboard: byKeyboard
        });

        needUpdate && _this._clear();
    ...
    _show: function() {
        var _this = this;
        if(!_this._popup || _this._popup.isShown()) {
            return;
        }

        _this._popup.show(_this._owner);
    },
    ...
    _getPos: function() {
        var _this = this,
            control = _this._control0,
            len = _this._val.length,
            sel;
    ...
    _buildPopup: function(suggestMods) {
        return {
            ...
            js: {
                position: {left: 0} /* Какой-то костыль… */
            },
    ...
    /* Функция _buildItems вынесена на уровень desktop. На уровне
     * touch-pad создана символическая ссылка на файл на уровне desktop.
     * На уровне touch-phone используется другой алгоритм группировки. */
});

/* ../../../../common.blocks/suggest/suggest.js end */
;
/* ../../../../touch.blocks/suggest/suggest.js begin */
BEM.decl('suggest', {
    ...
    onSetMod: {
        js: function() {
            ...
            _this._control.on('pointerup', _this._request.bind(_this));

            /* На iPad не возникает события blur/focusout при тапе
             * вне окошка с подсказками. */
            BEM.blocks['i-ua'].ipad &&
                BEM.DOM.doc.on('tap.suggest', function(e) {

                    if(!_this._popup ||
                        _this._popup.containsDomElem($(e.target)) ||
                        e.target === _this._control0) {
    ...
    /**
     * Выключает прослушивание события tap.suggest на элементе document.
     * Уничтожает сам блок.
     *
     * @public
     */
    destruct: function() {
        ...
        BEM.blocks['i-ua'].ipad && BEM.DOM.doc.off('tap.suggest');
    ...
    _create: function() {
        ...
        var content = _this._popup.elem('content');

        BEM.blocks['suggest-item'].on(content, 'pointerdown', function(e) {
            _this._onItemMouseDown(e.block);
        }).on(content, 'pointerup', function(e) { /* TODO Острова 2.0: Убрать. */
            _this._onItemMouseUp(e.block);
        }).on(content, 'tap', function(e) {
            _this._onSelectItem(e.block);
        });
...
/* ../../../../touch.blocks/suggest/suggest.js end */
;
/* ../../../../touch-pad.blocks/suggest/suggest.js begin */
BEM.decl('suggest', {
}, {
    /* FIXME Внимание!!! Копипаст из desktop.blocks!!! */

    /**
     * Создаёт элементы подсказки.
     *
     * @protected
     * @param {Array} items — данные от сервера
     * @param {Array} meta — мета-информация от сервера
    ...
    _buildItems: function(items, meta) {
        ...
        items.forEach(function(item) {
            var isArray = $.isArray(item),
                type = isArray ? item[0] : 'text',
                groupType, prefs, label;

            isArray && (prefs = item.concat().pop());
            $.isPlainObject(prefs) || (prefs = {});

            type || (type = 'text');
            BEM.blocks['suggest-item'].isItem(type) || (type = 'text');

            groupType = type;
            prefs.pers && (groupType = 'nah');

            /* После полного запуска группировок (group:1), можно оторвать. */
            !meta.group &&
                groupType !== 'nav' &&
                groupType !== 'nah' &&
                    (groupType = 'text');

            if(lastType !== groupType) {
                lastType = groupType;
                lastGroup = [];

                label = groupType === 'text'
                    ? _this._getTextLabel()
                    : BEM.I18N('suggest', groupType);

                bemjson.push({
                    block: 'suggest',
                    elem: 'group',
                    content: [
                        {elem: 'title', content: label},
                        {elem: 'items', content: lastGroup}
                    ]
                });
            }

            lastGroup.push($.extend({
                block: 'suggest-item',
                mods: {type: type},
                data: item
            }, prefs));
    ...
    /**
     * Возвращает заголовок для группы по-умолчанию
     *
     * Обычный заголовок — «Поиск», но сервисы могут выбрать другой.
     *
     * @protected
     * @returns {String}
     */
    _getTextLabel: function() {
        return BEM.blocks['i-global'].param('suggest-label') ||
               BEM.I18N('suggest', 'text');
    }
});

/* ../../../../touch-pad.blocks/suggest/suggest.js end */
;
...
;
/* ../../../../common.blocks/suggest/__provider/suggest__provider.js begin */
/**
 * Поставщик данных для саджеста с помощью AJAX.
 */
BEM.decl({name: 'suggest__provider', baseBlock: 'i-request_type_ajax'}, {
    ...
    get: function(part, pos, callback) {
        ...
        return this.__base({
            ...
            callback.call(this, {
                orig: data[0], /* Нужно для автодополнения. */
                items: data[1],
                meta: data[2]
            });
        });
...
/* ../../../../common.blocks/suggest/__provider/suggest__provider.js end */
;
/* ../../../../common.blocks/suggest-item/suggest-item.js begin */
/**
 * Вариант в саджесте.
 */
BEM.DOM.decl('suggest-item', {
    ...
    /**
     * Возвращает текст варианта в саджесте.
     *
     * @protected
     * @returns {String}
     */
    ...
    /**
     * Действие при выделении варианта подсказки.
     *
     * Метод вызывается, когда пользователь выделяет вариант подсказки с помощью
     * стрелочек вверх/вниз на клавиатуре. В блоке suggest это предусмотрено.
     *
     * @protected
     * @returns {Boolean} `true`, если есть необходимость обновить значение в поле ввода саджеста.
     */
    enter: function() {
        return true;
    },
    ...
    /**
     * Действие при выборе варианта подсказки.
     *
     * Метод вызывается, когда пользователь выбирает вариант подсказки с помощью
     * нажатия Enter или клика по нему. В блоке suggest это предусмотрено.
     *
     * @protected
     * @param {Boolean} byKeyboard `true`, если выбор подсказки сделан с помощью клавиатуры.
     * @returns {Boolean} `true`, если есть необходимость обновить значение в поле ввода саджеста.
     */
    ...
    /**
     * Флаги возможности обработки типов подсказок.
     *
     * Если `1`, значит данный тип подсказки поддерживается.
     *
     * @private
     * @type {Object}
     */
    ...
    /**
     * Проверяет, поддерживается ли данный тип подсказки блоком.
     *
     * @public
...
/* ../../../../common.blocks/suggest-item/suggest-item.js end */
;
/* ../../../../touch.blocks/suggest-item/suggest-item.js begin */
BEM.DOM.decl('suggest-item', {

/**
 * Прокси для DOM-события pointerdown.
 *
 * @event suggest-item#pointerdown
 * @param {$.Event} e Объект DOM-события.
 */

/**
 * Прокси для DOM-события pointerup.
 *
 * @event suggest-item#pointerup
 * @param {$.Event} e Объект DOM-события.
 */

/**
 * Прокси для DOM-события tap.
 *
 * @event suggest-item#tap
 * @param {$.Event} e Объект DOM-события.
 */
    ...
    live: function() {
        ...
        this.liveBindTo('pointerdown pointerup tap', function(e) {

            this.trigger(e.type, e);

        });

        /* Нужно для i-pressed-controller. */
        return false;
...
/* ../../../../touch.blocks/suggest-item/suggest-item.js end */
;
/* ../../../../common.blocks/suggest-item/_nav/suggest-item_nav_yes.js begin */
/**
 * Вариант в виде навигационной подсказки.
 *
 * @param {Object} params Параметры блока.
 * @param {String} params.val Ссылка для перехода на сайт.
 */
BEM.DOM.decl({block: 'suggest-item', modName: 'nav', modVal: 'yes'}, {

    val: function() {
        return this.elem('link').text();
    },
    ...
    select: function() {
        $('<form style="display:none" action="' + this.params.val + '" target="_blank"/>')
            .appendTo(BEM.DOM.scope)
            .submit()
            .remove();
    ...
    live: function() {
        ...
        this.liveBindTo('link nav', 'leftclick tap', function(e) {
            e.preventDefault();
        });
...
/* ../../../../common.blocks/suggest-item/_nav/suggest-item_nav_yes.js end */
;
...
BEM.DOM.decl('popup', {
        ...
        'visibility': {
            'visible': function() {
                ...
                if(this.hasMod('position', 'fixed')) {
                    // XXX: Костыль, вызывающий repaint при смене ориентации с открытой клавиатурой
                    this.bindToDoc('orientationchange', this._onOrientationChange);

                    // XXX: Костыль, вызывающий reflow при скрытии клавиатуры
    ...
    /**
     * XXX: Костыль для LEGO-9148
     * Вызывает repaint паранджи и `body`.
     *
     * @private
     */
    ...
    /**
     * XXX: Костыль для LEGO-9148
     * Вызывает reflow паранджи и `body`.
     *
     * @private
     */
...
/**
 * Смена значения в нативном контроле отслеживается с помощью DOM-события `input`.
 */
BEM.DOM.decl('input', {}, {
    ...
    live: function() {
        ...
        // Здесь live-инициализация по событиям `tap` на элементе `clear` и `input` на элементе `control`
        this
...
;
/* ../../../../touch-pad.blocks/input/input.js begin */
BEM.DOM.decl('input', {
    ...
    onSetMod: {
        ...
        'focused': function(modName, modVal) {
            ...
            // NOTE: Вызываем принудительную перерисовку компонента для исправления LEGO-9242
            var p = this.elem('box').css('z-index');
            this.elem('box').css('z-index', -1).css('z-index', p);
        }
...
/* ../../../../touch-pad.blocks/input/input.js end */
;
...
BEM.I18N.decl('suggest', {"fact": 'Gerçekler',"group": 'İpucu grubu',"nah": 'Önceki aramalar',"nav": 'Site',"text": 'Web',"traffic": 'Trafik',"weather": 'Hava durumu'}, {
"lang": "tr"
});

BEM.I18N.decl('suggest-item', {"quick-answer": 'Hızlı yanıt'}, {
"lang": "tr"
});
